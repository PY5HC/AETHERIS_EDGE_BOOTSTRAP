...
UNIT_TMP_DIR="$(mktemp -d)"; UNIT_TMP="$UNIT_TMP_DIR/aetheris-node.service"; REPORT="${AETHERIS_REPORT_PATH:-/tmp/aetheris-g3.6-live-apply-$(date -u +%Y%m%dT%H%M%SZ).report}"; STAGING_ROOT=""; RELEASE_PROMOTED=NO; UNIT_INSTALLED=NO; UNIT_ENABLED=NO; SERVICE_STARTED=NO; SERVICE_STOPPED=NO; UNIT_DISABLED=NO; UNIT_REMOVED=NO; STATE_PREEXISTING=NO; RUNTIME_PREEXISTING=NO; STATE_ACTION=NOT_NEEDED; RUNTIME_ACTION=NOT_NEEDED; RELEASE_PRESERVED=NO
rollback(){
  set +e
